...
CREATE TABLE public.t_template_send (
	id regclass NOT NULL DEFAULT nextval('t_template_send_seq'::regclass), -- 主键ID
	mobile text NULL,
	nationcode varchar NULL,
	app_id int4 NULL, -- 应用ID
	t_template_type varchar NULL, -- 0：普通短信  1 ：营销短信 （群发）
...
COMMENT ON COLUMN public.t_template_send.t_template_type IS '0：普通短信  1 ：营销短信 （群发）';
